<template>
  <section class="main_con">
    <div class="container">
      <div class="section p10">
        <div class="row">
          <div class="col-md-12 text-center fbcen">
            <div>
              <h1 v-if="err">Что-то пошло не так, попробуйте позже</h1>
              <h1 >ВАШ АККАУНТ АКТИВИРОВАН!</h1>
              <br />
              <h2>
                Учитесь! <br />
                Победа любит подготовку!
              </h2>
              <br />
              <router-link to="/login"><h1> ВОЙТИ</h1></router-link>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>
</template>

<script>
// @ is an alias to /src
import axios from "axios";
export default {
  name: "Home",
  components: {},
  data() {
    return {
      err: false,
      username: "",
      password: "",
      login_error: false,
      button_disabled: false,
    };
  },
  mounted() {
    axios
      .post(
        "https://education-api.gnt-it.ru/v1/auth/activation",
        {
          activationCode: this.$route.params.code,
          email: this.$route.params.email,
        },
        {}
      )
      .then((response) => {
        console.log(response.data);

        return false;

        localStorage.setItem("token", response.data.user.token);

        this.$store.state.token = response.data.user.token;
        this.$router.push("/");
      })
      .catch((error) => {
        this.login_error = true;
      });
  },
  methods: {
    gotoCur() {
      this.$router.push("/course-view/" + this.$store.state.byuingCur + "/1");
    },
  },
};
</script>

<style scoped>
@font-face {
  font-family: "Amsterdam Deco One";
  src: local("Amsterdam Deco One"),
    url("@/assets/fonts/AmsterdamDecoOne.woff") format("truetype");
}

@font-face {
  font-family: "Am";
  src: local("Am"), url("@/assets/am.ttf") format("truetype");
}

img {
  cursor: pointer;
}
.fbcen {
  min-height: 80vh;
  display: flex;
  align-items: center;
  justify-content: center;
}

h2 {
  font-family: "Am";
}

h1 {
  font-family: "Bertilion";
  font-style: normal;
  font-weight: 400;
  font-size: 96px;
  line-height: 98px;
}

.hund100 {
  margin: 10px 0;
  padding: 0px 10px;
}

.hund100 {
  width: 100%;
  height: 50px;
}

.hund50 {
  margin: 10px 0;
  font-weight: 400;
  width: 100%;
  height: 50px;
  color: black;
  font-size: 32px;
}

@media only screen and (min-width: 1000px) {
  .hund50 {
    width: 50% !important;
    height: 50px;
  }

  .p10 {
    padding: 100px 0;
  }
}

.main_con {
  min-height: 100vh;
  background-size: cover;
  background-image: url("@/assets/img/2.png");
}
</style>